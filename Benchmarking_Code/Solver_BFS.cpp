...
namespace Bucket_Partitioned_MDS
{
    ...
    {
        /*
        * get_routes: Helper function to construct randomized routes from MST using BFS
        * @param cvrp: CVRP instance
        * @param mst_adj: Adjacency list of MST 
        * @param bucket: Nodes in the bucket
        * @param routes: Routes constructed from random BFS order of MST
        ...
        */
        ...
        // Queue data structure for BFS (replaces stack)
        std::queue <node_t> q;

        // Starting BFS with depot
        node_t v                    = depot;
        ...
        while(!q.empty()) 
        {
            ...
            // If it's not the depot, we process it into the vehicle routes
            if (v_index != depot_index)
            {
                ...
                // Start or continue current route
            ...
            // Gather unvisited neighbors, shuffle them for randomization, and push to queue
            std::vector<node_t> neighbors;
            for(node_t neighbor_idx : mst_adj[v_index])
            {
            ...
            for(node_t neighbor_idx : neighbors)
            {
                q.push(neighbor_idx);
            }
        }
